...
@RequestScoped
public class DistribuidorListProducer {
	@Inject
	...
	private List<Lead> distribuidores;
	
	@Produces
	@Named
	public List<Lead> getDistribuidores()
	{
	...
	public void retrieveAllLeadsOrderedByNome() {
		...
		{
			distribuidores = em.createNamedQuery("Distribuidor.findAllSemUFCidade").getResultList();
		}
		else
		{
			...
			if(leadsId.size() > 0)
			{
				distribuidores = em.createNamedQuery("Distribuidor.findByIdsSemUFCidade").setParameter("leads", leadsId).getResultList();
			}
			else
			{
